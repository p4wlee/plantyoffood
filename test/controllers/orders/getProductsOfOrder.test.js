...
describe("getProductsOfOrder Controller", () => {
  ...
  // test di successo con lista prodotti associati all' ordine
  it("dovrebbe restituire 200 e la lista dei prodotti associati all'ordine", async () => {
    ...
    // seconda chiamata: restituisco prodotti associati
    const fakeProducts = [{ id: 1, nome: "Tofu" }];

    executeStub.onSecondCall().resolves([fakeProducts]);

    // eseguo controller
    await ordersController.getProductsOfOrder(req, res);
    ...
    // verifico che vengano restituiti i prodotti
